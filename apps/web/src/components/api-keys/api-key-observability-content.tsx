...
function ExternalCallListCard({ calls, loading, onCopy, window }: { calls: ExternalApiCallLogItem[]; loading: boolean; onCopy: (value: string, message: string) => void; window: ExternalApiObservabilityWindow }) {
  return (
    <Card className="grid gap-4 p-5">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><h2 className="text-sm font-semibold">最近外部调用</h2><StatusBadge tone="mock">{calls.length} 条</StatusBadge></div>
...
function QuotaWatchCard({ items, loading }: { items: Awaited<ReturnType<typeof getExternalApiObservability>>['quota_watch']; loading: boolean }) {
  return <Card className="grid gap-4 p-5"><h2 className="text-sm font-semibold">额度关注</h2>{loading ? <div className="grid gap-2">{Array.from({ length: 3 }).map((_, index) => <div className="h-16 rounded-md border bg-muted/30" key={index} />)}</div> : items.length === 0 ? <EmptyState description="暂无可观测的 API Key 额度数据。" title="暂无额度数据" /> : <div className="grid gap-2">{items.map((item) => <div className="rounded-md border bg-muted/20 p-3" key={item.api_key_id}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="truncate text-sm font-medium">{item.api_key_name}</div><div className="mt-1 font-mono text-xs text-muted-foreground">{item.masked_key}</div></div><StatusBadge tone={quotaRiskTone(item.risk_level)}>{quotaRiskLabel(item.risk_level)}</StatusBadge></div><div className="mt-2 grid gap-1 text-xs text-muted-foreground"><span>今日：{formatInteger(item.used_count_today)} / {item.daily_quota === null ? '不限' : formatInteger(item.daily_quota)}</span><span>剩余：{item.remaining_today === null ? '不限' : formatInteger(item.remaining_today)}</span><span>使用率：{item.usage_rate === null ? '未设额度' : formatPercent(item.usage_rate)}</span><span>最近使用：{formatDateTime(item.last_used_at)}</span></div></div>)}</div>}</Card>;
}
...
function SecurityDenialCard({ items, loading, onCopy, window }: { items: ExternalApiSecurityDenialItem[]; loading: boolean; onCopy: (value: string, message: string) => void; window: ExternalApiObservabilityWindow }) {
  return <Card className="grid gap-4 p-5"><h2 className="text-sm font-semibold">安全拒绝</h2>{loading ? <div className="grid gap-2">{Array.from({ length: 2 }).map((_, index) => <div className="h-20 rounded-md border bg-muted/30" key={index} />)}</div> : items.length === 0 ? <EmptyState description="当前窗口没有外部调用安全拒绝事件。" title="暂无拒绝事件" /> : <div className="grid gap-2">{items.map((item) => <div className="rounded-md border border-amber-200 bg-amber-50/60 p-3" key={item.event_id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-sm font-medium text-amber-900">{item.reason}</div><div className="mt-1 grid gap-1 text-xs text-amber-800"><span>来源：{item.source ?? '安全事件'}</span><span>密钥：{item.api_key_prefix ? `${item.api_key_prefix}****` : item.api_key_id ?? '未关联'}</span><span>Agent：{item.agent_id ?? '未知'}</span><span>时间：{formatDateTime(item.occurred_at)}</span></div></div><div className="flex shrink-0 flex-col gap-2">{item.trace_id ? <Button onClick={() => onCopy(item.trace_id ?? '', 'Trace ID 已复制。')} size="sm" type="button" variant="outline"><Copy className="size-4" />Trace</Button> : null}<Button asChild size="sm" type="button" variant="outline"><a href={`/audit?keyword=${encodeURIComponent(item.request_id)}&window=${window}`}>审计</a></Button></div></div></div>)}</div>}</Card>;
}
