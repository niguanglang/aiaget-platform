...
import { getCustomerSuccessAction } from '@/lib/api-client';

export function CustomerSuccessActionDetailContent({ actionId }: { actionId: string }) {
  ...
  const actionQuery = useQuery({
    queryKey: ['customer-success-action', actionId],
    queryFn: () => getCustomerSuccessAction(actionId),
  });
  ...
  return (
    <main className="relative mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:px-6">
      ...
      <section className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            {item.customer_name} / 负责人 {item.owner?.name ?? '未分配'} / 截止 {formatDateTime(item.due_at)} / 完成 {formatDateTime(item.completed_at)}
          </p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          {canWrite ? (
            <Button asChild>
              <Link href={`/customer-success-actions/${item.id}/edit`}>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
        <Card className="p-5">
          <h2 className="text-sm font-semibold">行动摘要</h2>
          <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{item.action_summary}</p>
        </Card>
        <Card className="p-5">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <DetailCard title="预期结果" value={item.expected_outcome} />
        <DetailCard title="执行记录" value={item.execution_notes} />
        <DetailCard title="阻塞风险" value={item.blocker_summary} />
        <DetailCard title="完成证据" value={item.completion_evidence} />
