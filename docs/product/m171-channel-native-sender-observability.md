...
`ChannelSenderDeliveryListItem` 同步暴露 `sender_mode` 和 `provider_api`，由投递 provider、状态和脱敏后的请求 URL 推导，避免把观测字段塞进真实请求体影响重试。`provider_api` 使用 shared-types 中的 `ChannelSenderProviderApi`，后端、前端和 IA 合约共用同一组枚举值，减少裸 `string` 扩散。
...
M63-7 负责回调入口的 raw body、原生验签/解密和快速 ACK。主动回复不再停留在异步骨架：M63-8 的 Channel Sender 已补齐企业微信原生发送、钉钉 webhook、飞书原生与 webhook fallback、Slack 原生与 webhook fallback、自定义 Webhook 等发送路径。M171 在此基础上收口观测字段、脱敏边界和逐渠道测试，确保 `sender_mode`、`provider_api` 只进入审计/事件，不进入真实渠道请求体。
