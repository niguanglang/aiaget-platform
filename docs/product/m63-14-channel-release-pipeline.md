...
在 `/channels/release/pipeline` 提供「渠道发布流水线与发布批次」独立子页面：
...
- 批次操作，全部需要二次确认：
  ...
  - 终止批次
- 发布控制入口：
  - `/channels/release/control` 承载审批申请、审批通过/拒绝、灰度比例和回滚动作。
  - 流水线批次动作后刷新发布控制、流水线、门禁、自动推进、自愈、复盘报告和各子页渠道选择器 query key。
- 记录展示：
  - 最近发布批次
  - 最近发布事件
