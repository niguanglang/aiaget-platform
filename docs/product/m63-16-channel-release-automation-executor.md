...
- 在 `/channels/release/automation` 提供“渠道自动推进执行器”独立子页面。
- 通过发布治理总览 `/channels/release` 和发布治理子导航进入自动推进子页面；审批、灰度和回滚动作由 `/channels/release/control` 承载。
- 支持：
  - 查看执行器状态
  - 查看门禁结论
  - 查看今日执行次数
...
- 选择渠道后能看到自动推进执行器面板。
- 保存策略后能刷新执行器、门禁、流水线、发布控制、自愈、复盘报告和真实存在的渠道选择器 query key。
- 演练模式下满足门禁也不会推进全量。
