...
P0-12 只验收和发布应用服务：`web`、`control-api`、`agent-runtime` 和 `agent-runtime-worker`。PostgreSQL、MinIO、Qdrant、OpenSearch、Temporal 和可观测性组件必须使用已批准的外部服务；不要在本 Runbook 中新增或启动中间件容器。
...
- 生产执行模式必须关闭本地兜底：`AGENT_RUNTIME_EXECUTION_MODE=runtime_only`，`KNOWLEDGE_WORKFLOW_MODE`、`AGENT_TEAM_WORKFLOW_MODE`、`CHANNEL_RELEASE_WORKFLOW_MODE`、`CHANNEL_RELEASE_SELF_HEALING_WORKFLOW_MODE`、`PLUGIN_ROLLBACK_WORKFLOW_MODE`、`PLUGIN_HOOK_WORKFLOW_MODE` 均为 `temporal`，并且 `RUNTIME_TEMPORAL_ENABLED=true`。
- `agent-runtime-worker` 必须随生产应用一起启动，并连接已批准的外部 Temporal；否则严格 `temporal` 模式下工作流无法执行。
- 真实密钥不进入 git。
...
构建和启动应用服务。生产严格 workflow 模式要求同时启用 `agent-runtime-worker`：

```bash
pnpm build:prod
python3 -m compileall apps/agent-runtime/app
docker compose -f deploy/docker-compose.production.yml --env-file .env.production --profile temporal-worker up -d control-api agent-runtime agent-runtime-worker web
```

如果只需要渲染或检查 Temporal Worker 配置，仍然不要启动中间件；使用 Compose config 离线检查即可：

```bash
docker compose -f deploy/docker-compose.production.yml --env-file .env.production --profile temporal-worker config
```
