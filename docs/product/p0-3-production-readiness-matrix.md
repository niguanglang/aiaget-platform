...
| 子任务 | 可并行 | 写入范围 | 验收点 |
| --- | --- | --- | --- |
| P0-11A workflow 模式配置修正 | 已完成 | `.env.production.example`、`deploy/docker-compose.production.yml`、`knowledge-task-dispatcher.service.ts`、`validate-production-env.mjs` | 生产默认模式统一为 `temporal`，生产校验拒绝可本地兜底模式 |
