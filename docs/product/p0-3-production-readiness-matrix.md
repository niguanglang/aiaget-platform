...
| P0-8 插件生态生产闭环 | 已完成 | 否 | 已补 Manifest 静态校验、自定义插件来源/sha256/签名存在性门禁、真实包下载与 sha256 计算、外部在线 verifier 强制验签门禁、前端完整性预检门禁、Tool Gateway 绑定预览、升级/回滚控制面闭环、Runtime/Temporal 回滚派发、受控 Hook 入队、Hook Runtime 执行、Hook 失败恢复、安装失败事件和卸载权限闭环；签名存在且策略要求验签时，未配置或不可用在线 verifier 必须阻断安装，metadata-only 仅用于审计展示，不计为验签通过；Hook 执行限定为 Manifest 生成工具并复用 Tool Gateway 审批/限流/安全/审计边界；内置 Sigstore/PGP SDK verifier 和插件包代码沙箱留到后续增强 | 纳入 P0-12 最终 smoke test；如后续允许插件包自定义代码运行，需要单独接插件沙箱和资源隔离 |
