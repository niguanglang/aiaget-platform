...
8. 签名验证适配：`PluginPackageIntegrityService` 增加可注入 `PluginPackageSignatureVerifier`，支持 `SIGSTORE`、`PGP`、`CUSTOM` 签名类型的结果归一、事件记录和安装阻断；当 Manifest 声明签名或验签信息时，必须配置可用的 `PLUGIN_SIGNATURE_VERIFIER_URL` 或等价 verifier，否则完整性校验结果为 `FAILED` 并阻断安装。metadata-only 仅作为审计展示状态，不计为验签通过。
...
生产环境中，凡插件包存在签名且安装策略要求验签，必须通过以下变量配置在线企业验签服务；未配置、不可达或超时时，安装前校验必须失败并阻断安装：
...
- 当前 P0 校验已经覆盖控制面静态门禁、真实包下载、sha256 计算、企业外部在线 verifier 强制验签门禁、安装前 Tool Gateway 绑定预览、控制面版本回滚、Runtime/Temporal 回滚派发、受控 Hook 入队、Hook Runtime 执行和 Hook 失败恢复；内置 Sigstore/PGP SDK verifier、真正插件代码沙箱属于后续增强。
- Hook 不执行第三方任意代码；当前执行目标限定为 Manifest 同步生成的 Tool Center 工具，并复用 Tool Gateway 的审批、限流、安全策略和审计边界。

## 下一批文件边界

- 内置签名验证实现：`apps/control-api/src/plugins/` 可在现有 `PluginPackageSignatureVerifier` 接口下继续增加 Sigstore/PGP SDK 适配；当前 P0 生产口径依赖企业在线 verifier 完成真实验签，签名存在且策略要求验签时，未配置在线 verifier 不得通过安装。
