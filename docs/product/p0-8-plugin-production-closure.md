...
8. 签名验证适配：`PluginPackageIntegrityService` 增加可注入 `PluginPackageSignatureVerifier`，支持 `SIGSTORE`、`PGP`、`CUSTOM` 签名类型的结果归一、事件记录和安装阻断；默认未配置时只记录签名元数据并标记跳过，配置 `PLUGIN_SIGNATURE_VERIFIER_URL` 后会调用企业外部验签服务执行真实验签。
...
验签服务返回结果会归一化为 `PluginPackageSignatureResult`。`status=FAILED`、HTTP 非 2xx、无效 JSON 或无效状态都会阻断安装，并写入 Manifest 校验事件 payload：
...
- 当前 P0 校验已经覆盖控制面静态门禁、真实包下载、sha256 计算、企业外部验签服务适配、安装前 Tool Gateway 绑定预览、控制面版本回滚、Runtime/Temporal 回滚派发、受控 Hook 入队、Hook Runtime 执行和 Hook 失败恢复；内置 Sigstore/PGP SDK verifier、真正插件代码沙箱属于后续增强。
- Hook 不执行第三方任意代码；当前执行目标限定为 Manifest 同步生成的 Tool Center 工具，并复用 Tool Gateway 的审批、限流、安全策略和审计边界。

## 下一批文件边界

- 内置签名验证实现：`apps/control-api/src/plugins/` 可在现有 `PluginPackageSignatureVerifier` 接口下继续增加 Sigstore/PGP SDK 适配；当前企业验签服务适配已经可通过生产环境变量启用。
