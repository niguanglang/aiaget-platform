...
6. 真实完整性校验：新增 `PluginPackageIntegrityService`，通过 HTTP/HTTPS 或已配置的 S3/MinIO 对象存储来源下载插件包，限制 25 MB 和 15 秒 HTTP 超时，计算 `actual_sha256` 并与 Manifest 声明比对；失败会阻断安装并写入事件 payload。
...
```json
{
  "source_url": "https://plugins.example.com/ticket-suite.tgz 或 minio://plugin-bucket/ticket-suite.tgz",
...
插件包 `package.source_url` 支持三类来源：

```text
https://plugins.example.com/ticket-suite.tgz
s3://plugin-bucket/releases/ticket-suite.tgz
minio://plugin-bucket/releases/ticket-suite.tgz
```

当使用 `s3://` 或 `minio://` 时，Control API 通过 S3-compatible SDK 从外部对象存储读取包内容，并继续执行 sha256 与签名校验。该能力不创建、不启动任何对象存储服务。

默认复用 `MINIO_ENDPOINT`、`MINIO_ROOT_USER`、`MINIO_ROOT_PASSWORD`、`MINIO_REGION`。如果插件包存放在独立桶或独立账号下，可以配置专用只读连接：

```text
PLUGIN_PACKAGE_OBJECT_STORAGE_ENDPOINT=https://plugin-packages.example.com
PLUGIN_PACKAGE_OBJECT_STORAGE_ACCESS_KEY=plugin-package-reader
PLUGIN_PACKAGE_OBJECT_STORAGE_SECRET_KEY=replace-this-reader-secret
PLUGIN_PACKAGE_OBJECT_STORAGE_REGION=us-east-1
```

生产环境只要设置了 `PLUGIN_PACKAGE_OBJECT_STORAGE_ENDPOINT`，就必须同时设置 access key、secret key 和 region；否则 `validate-production-env.mjs` 会阻断上线校验。
