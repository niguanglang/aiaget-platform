...
本文是 P0 收口阶段的历史并行执行计划，用于说明当时如何把 P0 模块拆成可并发执行的子任务。当前 P0 实现状态以 `docs/product/p0-3-production-readiness-matrix.md` 为准。

当前状态：

```text
P0-1 到 P0-12 已完成，均不再阻塞上线。
真实发布、备份恢复和目标环境业务 Smoke 按 P0-12 Runbook 在部署环境执行，不在代码阶段自动启动中间件或容器。
```

执行原则保持有效：所有后续新增子任务必须遵守 `P0-5` 统一事件与用量投影契约，不得新增或启动中间件/container，除非先征得用户确认。
