...
| 渠道与外部调用 | `/channels/*`、`/external/*` | `channel:publish:*`、外部 API Key scope | 渠道发布资源沿用 `CHANNEL` 数据范围；外部调用以 API Key 绑定租户、Agent、渠道 | 渠道核心发布资源已支持 `CHANNEL`；外部调用按 API Key 授权 | 外部调用支持 `Idempotency-Key`/`X-Idempotency-Key`，平台事件 `dedupeKey` 去重；渠道凭据轮换写 `channel.credential.rotated`；渠道投递审计统一脱敏 URL/Header/Body/响应敏感字段；外部回调配置签名材料后强制验签，签名或外部消息 ID 命中的重复回调会在 10 分钟窗口内拦截并写 `channel.callback.replay_blocked` | 已闭环 |
...
- 新增外部回调或渠道调用时，必须支持幂等键或可重复消费策略；签名回调应先验签再做重放窗口检查，避免重复会话、重复投递或重复扣费。
