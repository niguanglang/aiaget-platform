...
- Feature goal: 将 Agent 列表页从“列表 + 摘要详情 + 表单”混合页面调整为清晰的列表入口，新增/编辑进入独立路由，详情页继续承载完整信息、生命周期动作、资源绑定、版本、调试和审计。
...
- Required states: loading, empty, error, validation, disabled, success, permission-denied; 发布、停用、归档、回滚、删除、解除资源绑定必须有二次确认。
- Constraints: 不改后端接口；保留 `/agents` 和 `/agents/[id]`；新增 route-level create/edit 页面；列表页不展示完整详情、生命周期操作、资源绑定和配置说明；中文 UI 文案。
