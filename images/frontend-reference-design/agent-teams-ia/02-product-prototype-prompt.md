...
- Main task flow: user opens `/agent-teams`, scans metrics and filters teams, enters detail, then uses dedicated routes for create, edit, members, runs, run detail replay/compare, step drilldown, and report archives.
...
- Actions and states: search, filter, clear filters, create, open detail, edit, manage members, view runs, open run detail, inspect replay, compare previous run, open step detail, create handoff, save feedback, export report, create archive, download archive, request delete archive, approve/reject delete approval, loading, empty, error, validation, permission disabled, no previous run compare, high-impact confirmation dialogs.

Prototype requirements:
- Use low- to mid-fidelity wireframe style.
- Show one overview frame plus smaller route frames for create/edit, detail, members, runs, run detail, step detail, and report archives.
...
- The run detail frame must include separate wireframe blocks for 审计报告导出, 当前运行回放, 上一轮运行对比, 成员差异, Trace 图谱, 步骤时间线, 接力记录, and 反馈记录.
...
- Show confirmation dialog states for team deletion, member removal, run start, handoff creation, archive generation, archive delete request, and archive approval decisions.
