...
| Invoices | `billing-invoices-content.tsx` | `getBillingOverview`, invoice status mutation APIs, `BillingInvoiceItem` | Owns invoice status actions, detail panel and line-item parsing. |
...
| High-impact confirmation | `BillingConfirmDialog` in `billing-shared.tsx` | invoice, adjustment and subscription mutation flows | Locking/voiding invoices, applying/voiding adjustments and switching plans require explicit confirmation before mutation. |
