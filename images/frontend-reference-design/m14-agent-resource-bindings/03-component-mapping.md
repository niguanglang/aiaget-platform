...
| Binding removal confirmation | `apps/web/src/components/agents/agent-confirm-dialog.tsx` from `AgentBindingManager` | delete binding mutation pending state | Every resource binding delete opens a shared confirmation dialog before calling the API. |
