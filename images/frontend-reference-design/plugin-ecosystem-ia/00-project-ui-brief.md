...
- API 契约：复用 `getPluginOverview`、`listPluginMarket`、`listPluginInstallations`、`getPluginInstallation`、`installPlugin`、`updatePluginInstallation`、`enablePlugin`、`disablePlugin`、`upgradePlugin`、`uninstallPlugin`、`updatePluginHook`、`updatePluginMenuBinding`。
...
- 页面职责：`/plugins` 只承担查询、筛选、概览、市场安装入口和跳转入口；Manifest 展示、安装配置、安全审核、Hook 与菜单绑定编辑分别进入独立路由。
...
- 必备状态：加载、空列表、接口错误、无权限、操作中 disabled、成功提示、错误提示、表单 JSON 校验错误、只读权限禁用。
